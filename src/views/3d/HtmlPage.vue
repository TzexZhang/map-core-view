<!--
  @file 3D 地球 - 原生 HTML/JS 模式
  @description 模拟纯 HTML 场景下的 Cesium 3D 使用
-->
...
const status = ref('点击按钮创建 3D 地球')
let map: MapController | null = null

const log = (...args: unknown[]) => { console.log('[3D-HTML]', ...args); status.value = args.map(String).join(' ') }

function handleCreate() {
  const el = document.getElementById('html-3d-map')
  if (!el) { log('找不到容器'); return }
  MapController.create({ container: el, engine: EngineType.Cesium, initialView: { center: [116.397428, 39.90923], zoom: 10, pitch: -45 } })
    .then(m => { map = m; log('3D 地球创建成功（HTML 模式）') })
    .catch(e => log('创建失败:', e))
}

function handleAddLayer() {
  if (!map) return
  map.addLayer({ id: 'html-3d-points', type: LayerType.Vector })
  map.updateLayerData('html-3d-points', SAMPLE_GEOJSON_POINTS)
  log('矢量图层已添加')
}
...
<template>
  <div class="demo-page">
    <div class="demo-header">
      <h2>3D 地球 - 原生 HTML/JS</h2>
      <p class="desc">通过 document.getElementById 使用 Cesium 3D</p>
    </div>
    <div class="demo-body">
      ...
      <div class="map-area">
        <div id="html-3d-map" class="map-container"></div>
